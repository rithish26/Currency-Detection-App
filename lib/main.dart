...
//    classifyImage(widget.imagePath, total);
  ...
  Future<void> runTextToSpeech(String outputMoney, int totalMoney) async {
    ...
    if (outputMoney == "10 rupees") {
      String tot = totalMoney.toString();
      print(tot);
      String speakString = "ten rupees, Your total is now rupees, $tot";
    ...
    if (outputMoney == "20 rupees") {
      String tot = totalMoney.toString();
      print(tot);
      String speakString = "twenty rupees, Your total is now rupees, $tot";
      await flutterTts.setSpeechRate(0.8);
      await flutterTts.awaitSpeakCompletion(true);
      await flutterTts.speak(speakString);
    }
  ...
  classifyImage(String image) async {
    ...
    print(op);
    if (op != null) {
      if (op[0]["label"] == "10 rupees-samples") {
        total += 10;
        runTextToSpeech("10 rupees", total);
      }
      if (op[0]["label"] == "100 rupees-samples") {
        total += 100;
        runTextToSpeech("100 rupees", total);
      }
      if (op[0]["label"] == "20 rupees-samples") {
        total += 20;
        runTextToSpeech("20 rupees", total);
      }
      if (op[0]["label"] == "200 rupees-samples") {
        total += 200;
        runTextToSpeech("200 rupees", total);
      }
      if (op[0]["label"] == "2000 rupees-samples") {
        total += 2000;
        runTextToSpeech("2000 rupees", total);
      }
      if (op[0]["label"] == "50 rupees-samples") {
        total += 50;
        runTextToSpeech("50 rupees", total);
      }

      if (op[0]["label"] == "500 rupees-samples") {
        total += 500;
